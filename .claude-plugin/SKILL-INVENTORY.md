...
| **moku-worker** | moku worker, @moku-labs/worker, cloudflare durable object/queue/r2/d1/kv | Layer-2 Cloudflare Workers backend framework — **stub pending `moku-sync worker`** (registered in `moku-frameworks.md`, latest 0.4.0) |
| **moku-room** | moku room, @moku-labs/room, couch multiplayer, webrtc/state sync | Layer-2 couch-multiplayer framework built on `@moku-labs/web` — **stub pending `moku-sync room`** (registered, latest 0.1.1) |
...
> Only `moku-core` is broad; the other nine trigger narrowly (`moku-worker`/`moku-room` are stubs until
> `moku-sync` generates their catalogs). The toolkit also keeps **Reference Projects** in
