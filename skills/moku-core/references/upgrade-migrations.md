...
- **Detect:** `package.json` dependencies/devDependencies contain `@moku-labs/worker` AND its
  resolved/declared version `< frameworks[worker].knownVersion` in `moku-frameworks.md` (currently `0.9.2`).
- **Apply:**
...
- **Risk:** A minor/major `@moku-labs/worker` bump can change the Cloudflare plugin APIs/bindings.
  ...
  sites against the regenerated `plugin-index.md`. Mitigation: the plugin index is regenerated by
  `moku-sync` before this migration ships; review the release notes (`frameworks[worker].releaseSource`).
- **Rollback:** `git checkout -- package.json bun.lock && bun install`.
