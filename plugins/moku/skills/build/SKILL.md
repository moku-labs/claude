...
- **Comprehensive E2E + visual baseline is the final gate (web apps).** After the smoke test, run the comprehensive E2E + visual-baseline stage (`build-app.md` Step 7.5; full process in `${CLAUDE_PLUGIN_ROOT}/skills/moku-core/references/e2e-testing.md`): **every** screen/feature is tested in a real browser (Playwright) and pinned with a visual baseline, **every** bug or visual regression found is **fixed** and re-verified, and the build is not "done" until it is **green with full coverage**. It is **mandatory but skippable with an explicit, confirmed skip** (`AskUserQuestion` — "Run it (Recommended)" / "Skip — I confirm"); a skip is recorded prominently in the report ("⚠️ comprehensive E2E skipped — N screens unverified"), never silent. Spawn the `web-e2e-tester` agent; it gap-analyzes the **whole** app (incl. features built in earlier waves), so a feature added in any stage that lacks coverage is a gate failure. Confirm, don't assume.
