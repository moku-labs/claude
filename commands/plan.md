...
- **Run plan-checker agent BEFORE every user gate, and triage BLOCKERs BEFORE the gate is shown — users see validated plans only.** plan-checker's **Idiomatic Architecture** check (#10, rubric `moku-idioms.md`, worked reference `demos/tracker`) is part of this: its one BLOCKER is **I1 — a Layer-3 app that defines a framework** (calls `createCoreConfig`/`createCore` or depends on `@moku-labs/core` directly), which must be **fixed** (use `createApp` + the framework's `createPlugin`) before the gate. **Multiple `createApp` instances, composing multiple frameworks side-by-side, and folder splits are idiomatic — never a BLOCKER** (I2–I5 are WARNING nudges, not gate-blockers). This ordering is mandatory and is NOT relaxed by quick mode: the plan-checker → BLOCKER-triage → fix step must complete *before* the approval `AskUserQuestion` is presented, never after approval. In quick mode the three per-stage gates collapse to ONE combined gate — so run plan-checker on the final assembled plan and resolve every "Fix now" BLOCKER (per `plan-stages.md` Interactive Triage) *before* presenting that single gate. Presenting an approval while unresolved BLOCKERs exist is a defect (this was the recurring deviation in the multi-plan incident: gates were accepted, then BLOCKERs fixed after). If a BLOCKER is found after a gate was already shown, withdraw the gate, fix, re-run plan-checker, and re-present.
